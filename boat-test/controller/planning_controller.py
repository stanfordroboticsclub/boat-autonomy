...
class PlanningController(BaseController):
    ...
    def next_states(self, delta_x, delta_y, curr_state=[], prev_cost=0, cx=0, cy=0, ox=0, oy=0):
    ...
    def a_star(self, delta_x, delta_y):
        frontier = self.next_states(delta_x, delta_y)
        ...
        while True:
            ...
            possible = self.next_states(delta_x, delta_y, s_new, prev_cost=f_new, cx=pos_new[0], cy=pos_new[1])
            for k in possible:
                if k[1][-1] not in visited and not self.frontier_contains(frontier, k):
                    heapq.heappush(frontier, k)


    def draw(self, delta_x, delta_y, path):
        fig, ax = plt.subplots()
        grid_res = 0.5
    ...
    def select_action_from_state(self, env, state):
        ...
        path = self.a_star(delta_x, delta_y)

        # self.draw(delta_x, delta_y, path)

        return self.control(boat_angle, path[0][0], path[0][1], boat_speed, boat_ang_vel)
