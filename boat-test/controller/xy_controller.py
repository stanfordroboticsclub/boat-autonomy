...
class XYController(BaseController):
    ...
    def compute_jacobian(self, input, theta_i, ang_vel, x_targ, x_curr, y_targ, y_curr, v_i, v_cx, v_cy, t=1):
        # t = max(t - self.i_constant * self.running_error, 1e-3)

        delta_x = LatLon.dist(LatLon(y_targ, x_curr), LatLon(y_targ, x_targ)) * PIXELS_PER_METER
        if x_targ < x_curr:
            delta_x *= -1

        delta_y = LatLon.dist(LatLon(y_curr, x_targ), LatLon(y_targ, x_targ)) * PIXELS_PER_METER
        if y_targ < y_curr:
            delta_y *= -1

        x_curr, y_curr = latlon_to_xy(LatLon(y_curr, x_curr))
        x_targ, y_targ = latlon_to_xy(LatLon(y_targ, x_targ))
        v_i *= PIXELS_PER_METER
        ...
        dh_da = ((t**2) * np.sin(np.deg2rad(theta)) * ((t * (0.5 * a * t + v_i) * np.sin(np.deg2rad(theta))) - x_curr + x_targ) +
            (t**2) * np.cos(np.deg2rad(theta)) * ((t * (0.5 * a * t + v_i) * np.cos(np.deg2rad(theta))) - y_curr + y_targ))

        dh_dalpha = (((t**3) * ((0.5 * a * t) + v_i) * np.cos(np.deg2rad(theta)) * (dx_vel - x_curr + x_targ)) +
            (-(t**3) * ((0.5 * a * t) + v_i) * np.sin(np.deg2rad(theta)) * (dy_vel - y_curr + y_targ)))

        dh_da += self.dampening * 2 * (a*t + v_i) * t
        dh_dalpha += self.dampening * 2 * (alpha*t + ang_vel) * t

        return np.array([dh_da, dh_dalpha])
    ...
    def compute_objective(self, input, theta_i, ang_vel, x_targ, x_curr, y_targ, y_curr, v_i, v_cx, v_cy, t=1):
        ...
        return (dx_total) ** 2 + (dy_total) ** 2 + (self.dampening * (a*t + v_i)**2) + (self.dampening * (alpha*t + ang_vel)**2)
    ...
    def new_control(self, theta_i, ang_vel, x_targ, x_curr, y_targ, y_curr, v_i, v_cx, v_cy, use_accumulator=True):
        ...
        bounds = Bounds([-PIXELS_PER_METER * self.a_max, -PIXELS_PER_METER * self.max_alpha_mag], [PIXELS_PER_METER * self.a_max, PIXELS_PER_METER * self.max_alpha_mag])
        ...
        # solved = differential_evolution(obj_fun, bounds,
        #     (theta_i, ang_vel, x_targ, x_curr, y_targ, y_curr, v_i, v_cx, v_cy, t), maxiter=20, polish=True)

        x = solved.x

        solved = [np.clip(x[0], -PIXELS_PER_METER * self.a_max, PIXELS_PER_METER * self.a_max), np.clip(x[1], -PIXELS_PER_METER*self.max_alpha_mag, PIXELS_PER_METER*self.max_alpha_mag)]
    ...
    def select_action_from_state(self, env, state):
        ...
        # boat_x, boat_y = latlon_to_xy(LatLon(boat_y, boat_x))
        # waypoint = latlon_to_xy(LatLon(waypoint[1], waypoint[0]))
        ...
        return Action(2, [control[1], control[0] / PIXELS_PER_METER])
